...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <h5>Total: ${product.rating.count}</h5>
...
const loadSearchedProduct = () => {
  const value= document.getElementById("input-fieldSearch").value;
  let tempData = value ? productsArr.filter(x=> x.title.includes(value)) : productsArr;
  document.getElementById("all-products").innerHTML='' 
  showProducts(tempData);
